              ...
              <div class="card-body py-3">
                   <div class="row">
                       <div class="col-md-6">
                            ...
                            <div class="col-sm-6">
                                <label class="fw-bold fs-6 mb-2">{{__('admin.worktimes')}}</label>
                                <select name="worktimes[]" class="form-select form-select-solid" data-control="select2" id="" value="{{old('worktimes[]')}}" multiple>
                                    <option value="" disabled>{{__('admin.Choose Work Times')}}</option>
                                    @foreach($worktimes as $worktime)
                                    <option value="{{$worktime->id}}" @if(in_array($worktime->id,$selected_worktimes)) selected @endif >{{$worktime->start_time}} - {{$worktime->end_time}}</option>
                                    @endforeach
                                </select>
                                 @error('worktimes')
                               ...
                               {{--<div class="col-md-6">
                                   ...
                                   <div class="mb-5 {{ $errors->has('tax') ? 'has-error' : '' }}">
                                       <input type="text" name="tax" class="form-control form-control-solid" value="{{old('amount',$data->tax)}}" placeholder="{{__('admin.amount')}}"/>
                                ...
                                <label class="form-label fs-6 fw-bolder text-gray-700 mb-3">{{__('admin.type')}}</label>
                                 <div class="mb-5 {{ $errors->has('offer_type') ? 'has-error' : '' }}">
                                   <select name="tax_type"  id="type" class="form-select">
                                       ...
                                       <option value="amount"  @if($data->tax_type == "amount") selected @endif>{{__('admin.amount')}}</option>
                                       <option value="percent"  @if($data->tax_type ==  "percent") selected @endif>{{__('admin.percent')}}</option>
                                   </select>
                                </div>
                            </div>--}}
